...
def main():
    ...
    ik_commands = torch.zeros(robot.count, num_envs, device=robot.device)
    ...
    while simulation_app.is_running():
        ...
        # compute observation
        hand_pos = robot.data.ee_state_w[0, 0:3] - envs_positions[0]
        goal_pos = bowl.data.root_pos_w[0, :] - envs_positions[0]
        goal_to_hand_pos = hand_pos - goal_pos
        obs[0][0:3] = torch.tensor(hand_pos)
        obs[0][3:6] = torch.tensor(goal_pos)
        obs[0][6:9] = torch.tensor(goal_to_hand_pos)
        # predict the joint commands
        action, _states = model_reach.predict(obs)
